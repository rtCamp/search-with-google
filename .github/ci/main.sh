...
######################################################
######################## VARS ########################
SITE_NAME='searchwithgoogle.local'
...
# Create, setup the site
function create_and_configure_site () {

    ee site create $SITE_NAME --wp --php74 
    cd $SITE_ROOT/wp-content/plugins/
    mkdir search-with-google
    rsync -azh $GITHUB_WORKSPACE/ $SITE_ROOT/wp-content/plugins/search-with-google
    echo "127.0.0.1 $SITE_NAME" >> /etc/hosts
    ls
    wp plugin activate search-with-google --allow-root
